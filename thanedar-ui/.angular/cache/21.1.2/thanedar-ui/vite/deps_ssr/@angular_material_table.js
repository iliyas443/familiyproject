...
} from "./chunk-K4QVTA4L.js";
import {
  Platform
} from "./chunk-PXLDEMQQ.js";
...
var CdkTable = class _CdkTable {
  ...
  _headerRowStickyUpdates = new import_rxjs4.Subject();
  _footerRowStickyUpdates = new import_rxjs4.Subject();
  ...
  _dataStream = new import_rxjs4.Subject();
  ...
  _observeRenderChanges() {
    ...
    this._renderChangeSubscription = (0, import_rxjs4.combineLatest)([dataStream, this.viewChange]).pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe(([data, range]) => {
      this._data = data || [];
      this._renderedRange = range;
  ...
  _setupVirtualScrolling(viewport) {
    const virtualScrollScheduler = typeof requestAnimationFrame !== "undefined" ? import_rxjs4.animationFrameScheduler : import_rxjs4.asapScheduler;
    this.viewChange.next({
      start: 0,
      end: 0
    });
    viewport.renderedRangeStream.pipe((0, import_operators2.auditTime)(0, virtualScrollScheduler), (0, import_operators2.takeUntil)(this._onDestroy)).subscribe(this.viewChange);
    viewport.attach({
      dataStream: this._dataStream,
      measureRangeSize: (range, orientation) => this._measureRangeSize(range, orientation)
    });
    (0, import_rxjs4.combineLatest)([viewport.renderedContentOffset, this._headerRowStickyUpdates]).pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe(([offsetFromTop, update]) => {
    ...
    (0, import_rxjs4.combineLatest)([viewport.renderedContentOffset, this._footerRowStickyUpdates]).pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe(([offsetFromTop, update]) => {
      if (!update.sizes || !update.offsets || !update.elements) {
        return;
      }
      for (let i = 0; i < update.elements.length; i++) {
